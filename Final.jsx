...
function App() {
    ...
    return temp;
        ...
        setCatalogosCargados({
            tv: parsear(window.m3uTV || ""),
            vod: parsear(window.m3uVOD || "")
        });
    }, []);
    ...
    const buscarResena = async (video) => {
        ...
        // Sugerencias locales (misma categoría)
        const similares = catalogoActual[video.categoria]
            ?.filter(v => v.titulo !== video.titulo)
        ...
        try {
            ...
            if (data.results?.[0]) setExtraInfo(data.results[0]);
    ...
    <div className="inset-0 fixed bg-black z-[1200] p-12 flex flex-col overflow-hidden animate-in fade-in duration-500">
        ...
        {/* SECCIÓN DE CAPÍTULOS (Ahora aparece primero) */}
        ...
        {/* SECCIÓN DE SUGERENCIAS LOCALES (Ahora al final) */}
