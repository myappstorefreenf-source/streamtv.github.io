...
const CATALOGO = {
    ...
    thriller: [
        ...
        { titulo: "The ninth gate", url: "https://youtu.be/ikA0fVrvtiQ?si=3wzWPoit_xFU8tqV" },
...
// ----------------------------------------------------------------------
// 💡 COMPONENTE MenuLateral (Añadido)
// ----------------------------------------------------------------------

function MenuLateral({ onClose, menuBackRef }) {
    
    // Función nativa que debería estar disponible globalmente en Kotlin/Android
    const volverALaTV = () => {
        // Redirige el WebView a la URL deseada (tv.index.html)
        window.location.href = 'index.html';
onClose();
};
    return (
        <div 
            className="fixed inset-y-0 left-0 w-64 bg-gray-800 z-50 shadow-2xl p-6 flex flex-col items-start transition-transform duration-300"
            onKeyDown={(e) => {
        ...
        >
            ...
            <button
                ref={menuBackRef}
                onClick={volverALaTV}
                className="w-full text-left px-4 py-3 bg-red-700 text-white font-semibold rounded-lg shadow-md transition-all duration-200 hover:bg-red-600 focus:outline-none focus:ring-4 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-900"
                tabIndex={0}
                autoFocus 
            >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 inline mr-2" viewBox="0 0 20 20" fill="currentColor"><path d="M10.707 2.293a1 1 0 00-1.414 0l-7 7a1 1 0 001.414 1.414L4 10.414V17a1 1 0 001 1h2a1 1 0 001-1v-2a1 1 0 011-1h2a1 1 0 011 1v2a1 1 0 001 1h2a1 1 0 001-1v-6.586l.293.293a1 1 0 001.414-1.414l-7-7z" /></svg>
                Volver a la TV
            </button>
            
            <div className="mt-auto text-xs text-gray-400">Versión 1.0</div>
        </div>
    );
}
// ----------------------------------------------------------------------
...
function App() {
    ...
    const [mostrarMenuLateral, setMostrarMenuLateral] = React.useState(false); // 💡 NUEVO ESTADO
    
    const heroButtonRef = React.useRef(null); 
    const menuBackRef = React.useRef(null); // Ref al botón dentro del menú lateral
    ...
    // 3. MANEJO DEL BOTÓN ATRÁS EN EL MENÚ LATERAL (Nuevo)
    ...
    // Define la lógica de retroceso que Kotlin llama (orden jerárquico).
    // =========================================================================
    const consumeBackButton = React.useCallback(() => {
        
        // 1. Reproductor de Video (nivel más profundo)
        if (videoEnFocoUrl) {
            handleBack(); 
            return true; 
        }
        
        // 2. Cuadrícula de Videos (nivel intermedio)
        if (mostrarMasGrid) {
            handleCloseGrid(); 
            return true; 
        }
        
        // 3. Menú Lateral (nivel intermedio)
        if (mostrarMenuLateral) {
            handleCloseMenu(); 
            return true; 
        }
        
        // 4. Catálogo Principal (nivel más alto). Android/Kotlin gestiona la salida.
        return false; 
    }, [videoEnFocoUrl, mostrarMasGrid, mostrarMenuLateral, handleBack, handleCloseGrid, handleCloseMenu]);
    ...
    // LÓGICA DE NAVEGACIÓN D-PAD EN EL CATÁLOGO (Añadida la lógica de Overscan)
    const handleDpadNavigation = React.useCallback((event) => {
    
    // 1. VERIFICACIÓN DE ESTADO Y PREPARACIÓN
    // Ignorar la navegación si estamos en el reproductor, grid o menú lateral
    if (videoEnFocoUrl || mostrarMasGrid || mostrarMenuLateral) return; 

    const currentFocusedElement = document.activeElement;
    
    // Lista de todos los elementos enfocables en el catálogo
    const focusableElements = Array.from(
    ...
    event.preventDefault(); // Prevenir el scroll por defecto del navegador
    ...
    // --- 2. LÓGICA DE NAVEGACIÓN HORIZONTAL (ArrowRight / ArrowLeft) ---
    if (event.key === 'ArrowRight' || event.key === 'ArrowLeft') {
        ...
        if (event.key === 'ArrowLeft') {
            ...
            // 🔑 LÓGICA OVERSCAN: Si intentas ir a la izquierda del primer elemento (índice 0 -> -1)
            // Esto solo se activa si ya estás en el primer elemento enfocable (índice 0).
            if (nextIndex < 0) { 
                setMostrarMenuLateral(true);
                return; // Detener la ejecución, el menú se abre
            }
            
        } 
        
        // NAVEGACIÓN ESTÁNDAR: Se ejecuta si no se abrió el menú (y nextIndex >= 0) y para ArrowRight
    ...
    // --- 3. LÓGICA VERTICAL (ArrowDown / ArrowUp) ---
    else if (event.key === 'ArrowDown' || event.key === 'ArrowUp') {
        
        // Esta lógica depende de que el Hero Button sea índice 0
...
}, [videoEnFocoUrl, mostrarMasGrid, mostrarMenuLateral, setMostrarMenuLateral]);
// ======================================================================
    ...
    React.useEffect(() => {
        if (!videoEnFocoUrl && !mostrarMasGrid && !mostrarMenuLateral && !isReadyRef.current) {
            setTimeout(() => {
        ...
        // Cuando se cierra el menú lateral o el grid, el foco vuelve por handleCloseMenu/Grid
        // Cuando se abre el MenuLateral, el foco se maneja por su autoFocus
        // Cuando se cierra el reproductor, el foco vuelve por handleBack
    }, [videoEnFocoUrl, mostrarMasGrid, mostrarMenuLateral]);

    // Renderizado condicional del Reproductor o la Cuadrícula
    if (videoEnFocoUrl) {
        // ReproductorEnFoco debe estar definido en otro lugar
    ...
    return (
        <div className="p-4 md:p-8 max-w-7xl mx-auto min-h-screen bg-gray-900 text-white">
            ...
            {/* HERO BANNER (HeroBanner debe estar definido en otro lugar) */}
            <HeroBanner 
                ref={heroButtonRef}
            ...
            {Object.entries(CATALOGO).map(([categoria, videos]) => {
                ...
                return (
                    <div key={categoria} className="mb-10">
                        ...
                        {/* VideoCarousel debe estar definido en otro lugar */}
                        <VideoCarousel>
                            
                            {videosEnCarrusel.map((video, index) => (
                                <div key={index} className={cardClasses}>
                                   {/* ReproductorDeVideo debe estar definido en otro lugar */}
                                   <ReproductorDeVideo 
                                        titulo={video.titulo} 
                                        url={video.url} 
                                        onPlay={setVideoEnFocoUrl} 
                            ...
                            {videosRestantesCount > 0 && (
                                <div className={cardClasses}>
                                    {/* TarjetaMas debe estar definido en otro lugar */}
                                    <TarjetaMas 
            ...
            {mostrarMenuLateral && <MenuLateral onClose={handleCloseMenu} menuBackRef={menuBackRef} />}
        </div>
    );
}
