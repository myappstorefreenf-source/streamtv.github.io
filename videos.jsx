        ...
        <div className="bg-zinc-900 p-4 rounded-2xl border border-white/10 shadow-2xl w-[320px] select-none">
            <div className="grid grid-cols-6 gap-1 mb-2">
                {rows.map((row, rIdx) => row.map((letra, cIdx) => (
                    <div key={`${rIdx}-${cIdx}`} onClick={() => onKeyPress(letra)} className={`h-10 flex items-center justify-center rounded-lg font-bold text-sm cursor-pointer ${!isBottom && f === rIdx && c === cIdx ? 'bg-green-600 text-white scale-105' : 'bg-zinc-800 text-zinc-500'}`}>{letra}</div>
                )))}
            </div>
            <div className="flex gap-1">
                {['ESPACIO', 'BORRAR', 'LISTO'].map((t, i) => (
                    <div key={t} onClick={() => {if(i===0) onKeyPress(' '); if(i===1) onBackspace(); if(i===2) onClose();}} className={`flex-1 py-2 rounded-lg text-[10px] font-black text-center cursor-pointer ${isBottom && bCol === i ? 'bg-green-600 text-white' : 'bg-zinc-800 text-zinc-600'}`}>{t}</div>
                ))}
            </div>
        </div>
    );
...
const VideoCard = ({ video, esSeleccionado, id, esEpisodio, esVerMas, total, onClick }) => (
    <div id={id} onClick={onClick} className={`flex-shrink-0 transition-all duration-300 cursor-pointer ${esEpisodio ? 'w-28 h-28' : 'w-32'} ${esSeleccionado ? 'scale-95 ring-4 ring-green-600 z-10 opacity-100' : 'opacity-90'}`}>
...
function App() {
    ...
    const [focoZona, setFocoZona] = useState('visor');
    ...
    const lanzarVideoNativo = (url, titulo) => {
        ...
        else console.log("Reproduciendo:", url);
    };

    const handleCerrarVista = () => {
        if (vistaActual.fromGrid) setVistaActual({ tipo: 'grilla', data: vistaActual.fromGrid });
        else { setVistaActual({ tipo: 'home', data: null }); setFocoZona('grid'); setIndiceAux(0); }
    };
    ...
    }, [filaActiva, columnaActiva, vistaActual, focoZona, indiceAux, rangoCapitulos, mostrarTeclado]);
            ...
            if (vistaActual.tipo === 'home') {
                ...
                if (filaActiva !== -1) {
                    ...
                    if (isEnter) {
                        ...
                        else { setVistaActual({ tipo: 'detalle', data: { info: items[columnaActiva], items } }); setFocoZona('visor'); }
                    }
                ...
                if (isEnter) setVistaActual({ tipo: 'detalle', data: { info: vistaActual.data.items[indiceAux], items: vistaActual.data.items }, fromGrid: vistaActual.data });
            } else if (vistaActual.tipo === 'detalle') {
                const esSerie = vistaActual.data.info.categoria.toUpperCase().includes("SERIES");
                const current = esSerie ? vistaActual.data.items[(rangoCapitulos * 10) + indiceAux] : vistaActual.data.info;
                
                if (focoZona === 'visor') {
                    if (isEnter) lanzarVideoNativo(current.url, current.titulo);
    ...
    return (
        <div className="fixed inset-0 bg-black text-white font-sans overflow-hidden select-none">
            ...
            {vistaActual.tipo === 'home' && (
                <div className="h-full overflow-y-auto p-8 md:p-12 no-scrollbar">
                    <div className="flex justify-between items-center mb-12">
                        <div>
                            <h1 className="text-5xl font-black text-green-500 italic leading-none">HOOD</h1>
                            <p className="text-[10px] tracking-[0.3em] text-zinc-600 font-bold uppercase">Interface Premium</p>
                        </div>
                        <div className="relative">
                            <div id="fake-search" onClick={() => setMostrarTeclado(true)} className={`w-64 px-5 py-3 rounded-xl border-2 transition-all flex justify-between items-center cursor-pointer ${filaActiva === -1 ? 'border-green-600 bg-zinc-800 scale-105' : 'border-white/10 bg-zinc-900'}`}>
                                <span className="truncate text-sm text-zinc-500">{busqueda || "Buscar..."}</span>
                    ...
                    {categoriasKeys.map((cat, fIdx) => (
                        <div key={cat} className="mb-14">
                            <h2 className={`text-lg font-bold mb-4 uppercase ${filaActiva === fIdx ? 'text-green-500' : 'text-zinc-800'}`}>{cat}</h2>
                            <div className="flex gap-6 overflow-x-auto no-scrollbar py-2 touch-pan-x">
                                {catalogoFiltrado[cat].slice(0, 11).map((v, cIdx) => (
                                    cIdx < 10 ? 
                                    <VideoCard key={cIdx} id={`item-${fIdx}-${cIdx}`} video={v} esSeleccionado={filaActiva === fIdx && columnaActiva === cIdx} onClick={() => { setVistaActual({ tipo: 'detalle', data: { info: v, items: catalogoFiltrado[cat] } }); setFocoZona('visor'); }} /> :
            ...
            {vistaActual.tipo === 'grilla' && (
                <div className="h-full overflow-y-auto p-12 bg-zinc-950 no-scrollbar">
                    <button onClick={handleCerrarVista} className="mb-8 text-zinc-500 font-bold uppercase tracking-widest hover:text-white transition-all">← Volver al inicio</button>
                    <div className="grid grid-cols-3 md:grid-cols-6 gap-8 pb-32">
                        {vistaActual.data.items.map((v, i) => <VideoCard key={i} id={`grid-item-${i}`} video={v} esSeleccionado={indiceAux === i} onClick={() => setVistaActual({ tipo: 'detalle', data: { info: v, items: vistaActual.data.items }, fromGrid: vistaActual.data })} />)}
                    </div>
                </div>
            )}

            {/* VISTA DETALLE CON VISUALIZADOR */}
            {vistaActual.tipo === 'detalle' && (
                <div className="fixed inset-0 bg-zinc-950 z-[100] p-6 md:p-10 flex flex-col no-scrollbar overflow-hidden">
                    <div className="flex flex-col md:flex-row gap-10 mb-6">
                        {/* INFO IZQUIERDA */}
                        <div className="flex-1">
                            <button onClick={handleCerrarVista} className="bg-zinc-800 px-6 py-2 rounded-lg text-[10px] font-black uppercase mb-6 hover:bg-white hover:text-black transition-all">← Regresar</button>
                            <h2 className="text-3xl md:text-5xl font-black uppercase italic mb-4 leading-none">{vistaActual.data.info.titulo}</h2>
                            <span className="bg-green-600/20 text-green-500 px-4 py-1 rounded-full text-xs font-bold uppercase mb-8 inline-block">{vistaActual.data.info.categoria}</span>
                            <div className="hidden md:block text-zinc-500 text-sm max-w-md uppercase font-bold opacity-50">Contenido disponible para reproducción en ExoPlayer Nativo. Presione el visor para iniciar.</div>
                        </div>

                        {/* VISUALIZADOR (EL PUENTE) */}
                        <div 
                            id="visor-container" 
                            onClick={() => {
                                const esSerie = vistaActual.data.info.categoria.toUpperCase().includes("SERIES");
                                const current = esSerie ? vistaActual.data.items[(rangoCapitulos * 10) + indiceAux] : vistaActual.data.info;
                                lanzarVideoNativo(current.url, current.titulo);
                            }}
                            className={`relative w-full md:w-[540px] aspect-video bg-zinc-900 rounded-3xl overflow-hidden border-4 transition-all duration-300 cursor-pointer group ${focoZona === 'visor' ? 'border-green-600 scale-105 shadow-2xl shadow-green-600/20' : 'border-white/5'}`}
                        >
                            <img src={vistaActual.data.info.logo} className="absolute inset-0 w-full h-full object-cover opacity-10 blur-xl" />
                            <img src={vistaActual.data.info.logo} className="relative z-10 w-full h-full object-contain p-12 drop-shadow-2xl" />
                            <div className="absolute inset-0 z-20 flex items-center justify-center bg-black/40 group-hover:bg-black/20 transition-all">
                                <div className={`w-24 h-24 rounded-full flex items-center justify-center transition-all ${focoZona === 'visor' ? 'bg-green-600 scale-110' : 'bg-white/10'}`}>
                                    <span className="text-4xl ml-2 text-white">▶</span>
                                </div>
                            </div>
                            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-30 bg-black/80 px-6 py-2 rounded-full text-[10px] font-black text-white uppercase tracking-widest shadow-2xl border border-white/10">
                                {focoZona === 'visor' ? 'PRESIONA OK PARA EMPEZAR' : 'REPRODUCIR'}
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="mt-auto pt-6 border-t border-white/5">
                            <div className="flex gap-2 overflow-x-auto no-scrollbar mb-4">
                                {Array.from({ length: Math.ceil(vistaActual.data.items.length / 10) }).map((_, i) => (
                                    <div key={i} id={`range-${i}`} onClick={() => setRangoCapitulos(i)} className={`px-5 py-2 rounded-xl text-[10px] font-black cursor-pointer transition-all ${rangoCapitulos === i ? 'bg-green-600 text-white' : 'bg-zinc-900 text-zinc-600'} ${focoZona === 'selector' && rangoCapitulos === i ? 'ring-2 ring-white' : ''}`}>
                                        {i * 10 + 1}-{Math.min((i + 1) * 10, vistaActual.data.items.length)}
                                    </div>
                                ))}
                            </div>
                            <div className="flex gap-6 overflow-x-auto no-scrollbar py-4 touch-pan-x">
                                {vistaActual.data.items.slice(rangoCapitulos * 10, (rangoCapitulos + 1) * 10).map((v, i) => (
                                    <VideoCard key={i} id={`cap-${i}`} video={{...v, num: (rangoCapitulos * 10) + i + 1}} esSeleccionado={focoZona === 'grid' && indiceAux === i} esEpisodio={true} onClick={() => { setIndiceAux(i); lanzarVideoNativo(v.url, v.titulo); }} />
                                ))}
                            </div>
                        </div>
                    )}
